      ...
      {showStars ? (
        ...
        >
          {STAR_VALUES.map((value) => {
            ...
            return (
              ...
              >
                <StarIcon filled={filled} className={`h-6 w-6 ${filled ? 'text-sand' : 'text-line'}`} />
              </button>
            );
          })}
        ...
        <div className="mt-4 flex flex-wrap items-center justify-between gap-3 border border-line bg-surface-muted px-4 py-3">
          <p className="flex items-center gap-1.5 text-body-sm text-strong" aria-live="polite">
            <StarIcon filled className="h-4 w-4 shrink-0 text-sand" />
        ...
        {count === 0 ? (
          ...
          <p className="flex flex-wrap items-baseline gap-x-2 gap-y-0.5 text-body-sm">
            <span className="font-mono text-2xl font-semibold tabular-nums text-strong">
              {average?.toFixed(1)} <StarIcon filled className="inline h-4 w-4 -translate-y-0.5 text-sand" />
            </span>
