...
export const tr = {
  ...
  // ─── Manual route builder ("Rotam") ───────────────────────────
  route: {
    navLabel: 'Rotam',
    stopsInRoute: (n: number) => `${n} durak rotanda`,
    viewRoute: 'Rotanı görüntüle',
    builderTitle: 'Rotanı Oluştur',
    draftSubtitle: 'Eklediğin yerleri sırala, adlandır ve kaydet.',
    namePlaceholder: 'Rotana bir isim ver (örn. Girne\'de Bir Gün)',
    unnamedRoute: 'Adsız Rota',
    save: 'Rotayı Kaydet',
    saved: 'Kaydedildi',
    saving: 'Kaydediliyor…',
    ...
    renamed: 'Kaydedildi ✓',
    delete: 'Rotayı Sil',
    deleteConfirmTitle: 'Bu rotayı silmek istediğine emin misin?',
    deleteConfirmBody: (name: string) => `"${name}" kalıcı olarak silinecek. Bu işlem geri alınamaz.`,
    ...
    remove: 'Kaldır',
    addMore: 'Yer Ekle',
    addMorePlaceholder: 'Yer ara…',
    addMoreEmpty: 'Sonuç bulunamadı.',
    addMoreAlreadyIn: 'Rotada',
    stopCount: (n: number) => `${n} durak`,
    emptyDraftTitle: 'Rotan henüz boş',
    emptyDraftBody: 'Gezmek istediğin yerlerde "Rotaya Ekle" butonuna bas — buradan sıralayıp kaydedebilirsin.',
    browsePlaces: 'Yerlere Göz At',
    savedRoutesTitle: 'Rotalarım',
    savedRoutesEmpty: 'Henüz kaydedilmiş bir rotanız yok.',
    savedRoutesEmptyHint: 'Bir yerin sayfasında "Rotaya Ekle"ye basarak rota oluşturmaya başlayın.',
    updatedAt: (date: string) => `Son güncelleme: ${date}`,
    editRoute: 'Rotayı Düzenle',
    abstractRouteNote: 'Duraklar arası bağlantı gerçek bir yol güzergâhı değildir — sıralamayı gösteren soyut bir çizgidir.',
    segmentDistance: (km: number) => `~${km < 1 ? Math.round(km * 1000) + ' m' : km.toFixed(1) + ' km'} · kuş uçuşu`,
    segmentTime: (min: number) => `~${min} dk · araçla, tahmini`,
  ...
  rating: {
    ...
    puaniLabel: 'Gezeceyik Puanı',
    empty: 'Henüz puanlanmadı.',
    emptyCta: 'İlk değerlendiren sen ol.',
    reviewCount: (n: number) => `${n} değerlendirme`,
    starLabel: (value: number, meaning: string) => `${value} yıldız — ${meaning}`,
    myRatingLabel: (value: number) => `Senin puanın: ${value} yıldız`,
    saved: 'Kaydedildi',
